...
class Model(cs.SlikPlugin):
    ...
    def __init__(self,
        ...
        self.b_l = param(-3.1592766803303469, 0.02)
        ...
        if self.second_order:
            ...
            self.b2_l = param(0, 0.01)
            self.bz = param(0,0.01)
            self.bz_l = param(0,0.01)
            self.bR = param(0,0.01)
            self.bR_l = param(0,0.01)
    ...
    def __cost__(self):
        ...
        '''
        self.cepheids['R_field'] = 0.39
        if self.varyR:
            for field in self.fields.groups.keys():
                self.cepheids['R_field'] += array(self.cepheids.Field == field)*self['R_'+field] 


        self.cepheids.loc[:,'Extinction'] = (self.cepheids.VminusI
                           - self.C0
                           - ((self.cepheids.Period >= 10) *self.CP_h * (log10(self.cepheids.Period) - 1) + 
                              (self.cepheids.Period < 10) * self.CP_l * (log10(self.cepheids.Period) - 1)
                            )
        ...
        if self.color_cutoff:
            ...
            determinant = sum(log(cepheids.err**2+ 0.08**2*cepheids.R_field**2)) + (cepheids.err.size)*log(2*pi)
        else:
        ...
        yi = cepheids.m_H
               ...
               + (log10(cepheids.Period) - 1) * (array(cepheids.Period < 10) * self.b_l
                                                      + array(cepheids.Period >= 10) * self.b
                                                      )
               + cepheids.R_field * self.cepheids['Extinction']
               + mu_cephs
        ...
        if self.second_order:
            fi += ((log10(cepheids.Period)-1)**2 * (self.b2_l * (cepheids.Period < 10) 
                                                        + self.b2 * (cepheids.Period >= 10)
                                                       )
                   +(log10(cepheids.Period) -1)*(cepheids.Z - 8.9)*(self.bz_l * (cepheids.Period < 10)
                                                                              +self.bz * (cepheids.Period >= 10)
                                                                              )
                   +(log10(cepheids.Period) -1) * self.cepheids['Extinction'] *(self.bR_l * (cepheids.Period < 10)
                                                                              +self.bR * (cepheids.Period >= 10)
                                                                              )
                   +(cepheids.Z - 8.9) * self.cepheids['Extinction'] * self.zR
                   )
            
        if self.correlated_errs:
            cov = pd.DataFrame(diag(cepheids.err**2 + 0.08**2*cepheids.R_field**2), 
